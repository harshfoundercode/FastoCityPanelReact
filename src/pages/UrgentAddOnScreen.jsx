...
export const UrgentAddOnScreen = () => {
    ...
    const handleCartQtyInputChange = (index, e) => {
        let value = parseInt(e.target.value);
        if (isNaN(value)) value = 1;
        value = Math.max(1, Math.min(9999, value));
        ...
        setCart(prev => {
            const newCart = [...prev];
            newCart[index] = { ...newCart[index], qty: value };
            return newCart;
        });
    };
                                            ...
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="flex items-center justify-between mt-2">
                                                        <QtyControl 
                                                            qty={item.qty} 
                                                            onChange={(d) => changeCartQty(index, d)} 
                                                            onInputChange={(e) => handleCartQtyInputChange(index, e)}
                                                            small 
                                                        />
                                                    </div>
                                                </div>
...
// Quantity Control Component with Input Field
const QtyControl = ({ qty, onChange, onInputChange, small }) => (
    <div className={`flex items-center border border-gray-200 rounded-lg overflow-hidden ${small ? 'h-7' : 'h-9'}`}>
        <button
            onClick={() => onChange(-1)}
            className={`${small ? 'w-7 h-7' : 'w-9 h-9'} flex items-center justify-center hover:bg-gray-100 text-red-400 transition-colors`}
        >
            <Minus size={small ? 14 : 16} />
        </button>
        <input
            ...
            className={`${small ? 'w-12 h-7 text-sm' : 'w-14 h-9 text-base'} bg-gray-50 text-center font-extrabold text-green-800 outline-none focus:ring-2 focus:ring-green-200 border-x border-gray-200`}
            min="1"
            max="9999"
        />
        <button
            onClick={() => onChange(1)}
            className={`${small ? 'w-7 h-7' : 'w-9 h-9'} flex items-center justify-center hover:bg-gray-100 text-green-600 transition-colors`}
        >
            <Plus size={small ? 14 : 16} />
        </button>
    </div>
