...
const TransferCard = ({ transfer, expanded, onToggle, onAccept, getStatusLabel, getStatusColor, formatDate, getTotalQty, getTotalReceived }) => {
  ...
  return (
    <motion.div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      {/* Comparison Summary */}
      {transfer.comparison && transfer.comparison.length > 0 && expanded && (
        <div className="px-3.5 pb-3">
          <p className="text-xs font-semibold text-gray-600 mb-1.5">Summary:</p>
          <div className="flex flex-wrap gap-1.5">
            {transfer.comparison.map((item, idx) => (
              <span key={idx} className="text-xs bg-gray-100 px-2 py-1 rounded flex items-center gap-1">
                <span className="font-medium">{item.product_name}</span>
                <span className="text-green-600">{item.received_qty}/{item.sent_qty}</span>
                {item.dispute_qty > 0 && (
                  <span className="text-orange-500">({item.dispute_qty} defective)</span>
                )}
                {item.missing_qty > 0 && (
                  <span className="text-red-500">({item.missing_qty} missing)</span>
                )}
              </span>
            ))}
          </div>
...
const VariantRow = ({ variant }) => {
  ...
  // Handle dispute images - API provides array directly
  const disputeImages = variant.dispute_images || [];
  const hasDispute = disputeQty > 0;
  const hasDisputeImages = hasDispute && disputeImages.length > 0;
      ...
      {hasDispute && (
        <div className="mt-2 pt-2 border-t border-gray-100">
          ...
          {hasDisputeImages ? (
            ...
            <div className="text-xs text-yellow-600 flex items-center gap-1">
              <AlertTriangle size={14} />
              No images uploaded for defective items
            </div>
          )}
        </div>
      )}
  ...
  useEffect(() => {
    ...
    (transfer.items || []).forEach(item => {
      (item.variants || []).forEach(variant => {
        ...
        // Get existing dispute images - handle array directly
        let existingImages = variant.dispute_images || [];
    ...
    try {
      const items = Object.values(variantData).map(data => {
        ...
        return {
          ...
          missing_qty: data.missing_qty,
          dispute_qty: data.dispute_qty,
          dispute_images: disputeImages, // Send as array
        };
      });
