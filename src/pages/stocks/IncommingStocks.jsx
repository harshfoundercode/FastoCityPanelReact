...
// //           {/* Remark */}
// //           <div>
// //             <label className="text-xs font-semibold text-gray-700 mb-1.5 block">Remark</label>
// //             <textarea
// //               value={remark}
// //               onChange={(e) => setRemark(e.target.value)}
// //               rows={2}
...
// // ── Variant Row with Dispute Images Display ───────────────────────────────────
// const VariantRow = ({ variant }) => {
//   const sentQty = parseInt(variant.quantity) || 0;
//   const receivedQty = parseInt(variant.received_qty || variant.receivedQty) || 0;
//   const missingQty = parseInt(variant.missing_qty || variant.missingQty) || 0;
...
//   // Get dispute images (could be array or string)
//   const disputeImages = variant.dispute_images || variant.disputeImage || [];
//   const hasDisputeImages = disputeQty > 0 && disputeImages.length > 0;
...
//       {/* Display Dispute Images */}
//       {hasDisputeImages && (
//         <div className="mt-2 pt-2 border-t border-gray-100">
//           <p className="text-xs font-semibold text-gray-600 mb-2 flex items-center gap-1">
//             <Camera size={12} />
//             Dispute Images ({disputeImages.length})
//           </p>
//           <div className="flex flex-wrap gap-2">
//             {(Array.isArray(disputeImages) ? disputeImages : [disputeImages]).map((img, idx) => (
...
// // ── Multiple Image Upload Component ───────────────────────────────────────────
// const MultipleImageUpload = ({ images, onAddImages, onRemoveImage, maxImages = 5, readOnly = false }) => {
//   const handleFileSelect = (e) => {
//     if (readOnly) return;
...
//   const [variantData, setVariantData] = useState({});
//   const [isSubmitting, setIsSubmitting] = useState(false);
//   const isViewOnly = parseInt(transfer.status) !== 0; // If not pending, it's view-only
...
//         // Get existing dispute images if any
//         let existingImages = variant.dispute_images || variant.disputeImage || [];
...
//         missing_qty: data.missing_qty,
//         dispute_qty: data.dispute_qty,
//         dispute_images: data.dispute_images || [],
//       }));
...
//                       </>
//                     ) : (
//                       // View-only mode - just show the quantities and images
//                       <>
...
//                         {/* Display existing dispute images */}
//                         {disputeQty > 0 && images.length > 0 && (
//                           <div className="mt-2 pt-2 border-t border-gray-100">
//                             <p className="text-xs font-semibold text-gray-600 mb-2 flex items-center gap-1">
//                               <Camera size={12} />
//                               Defective Images ({images.length})
//                             </p>
//                             <div className="flex flex-wrap gap-2">
//                               {images.map((img, idx) => (
//                                 <div key={idx} className="relative group">
...
// ── Variant Row with Dispute Image Display ───────────────────────────────────
const VariantRow = ({ variant }) => {
  const sentQty = parseInt(variant.quantity) || 0;
  const receivedQty = parseInt(variant.received_qty || variant.receivedQty) || 0;
  const missingQty = parseInt(variant.missing_qty || variant.missingQty) || 0;
  ...
  // The image field in variant contains the dispute/defective product image
  const disputeImage = variant.image || null;
  const hasDispute = disputeQty > 0;
  const hasDisputeImage = hasDispute && disputeImage;
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-2.5">
      ...
      {/* Display Dispute/Defective Image */}
      {hasDispute && (
        <div className="mt-2 pt-2 border-t border-gray-100">
          <p className="text-xs font-semibold text-gray-600 mb-2 flex items-center gap-1">
            <Camera size={12} />
          ...
          {hasDisputeImage ? (
            <div className="relative group inline-block">
              <img
                src={disputeImage}
                alt="Defective product"
                className="w-32 h-32 rounded-lg object-cover border border-gray-200 cursor-pointer hover:opacity-90 transition-opacity"
                onClick={() => window.open(disputeImage, '_blank')}
              />
              <button
                onClick={() => window.open(disputeImage, '_blank')}
                className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 rounded-lg flex items-center justify-center transition-opacity"
...
const AcceptTransferDialog = ({ transfer, onClose, onSuccess }) => {
    ...
    (transfer.items || []).forEach(item => {
      (item.variants || []).forEach(variant => {
        ...
        data[key] = {
          ...
          missing_qty: variant.missing_qty || 0,
          dispute_qty: variant.dispute_qty || 0,
          dispute_image: variant.image || null, // Store the existing dispute image
          maxQty: sentQty,
          productName: item.product_name || item.productName,
          variantValue: variant.value,
  ...
  const uploadImage = async (key, file) => {
    ...
    updateVariant(key, 'uploadingImage', true);
    ...
    try {
      ...
      if (data.secure_url) {
        ...
        updateVariant(key, 'dispute_image', null);
      ...
      updateVariant(key, 'uploadingImage', false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    for (const [key, data] of Object.entries(variantData)) {
      ...
      if (data.dispute_qty > 0 && !data.dispute_image) {
        toast.error(`Please upload a defective image for ${data.productName} - ${data.variantValue}`);
        return;
      }
      if (data.uploadingImage) {
    ...
    try {
      const items = Object.values(variantData).map(data => ({
        ...
        missing_qty: data.missing_qty,
        dispute_qty: data.dispute_qty,
        dispute_image: data.dispute_image || null,
      }));
          ...
          {(transfer.items || []).map((item) => (
            <div key={item.productid} className="bg-gray-50 rounded-xl border border-gray-200 p-3">
              ...
              {(item.variants || []).map((variant) => {
                ...
                const disputeQty = data.dispute_qty;
                const missingQty = data.missing_qty;
                const disputeImage = data.dispute_image;
                const isUploading = data.uploadingImage;
                    ...
                    {!isViewOnly ? (
                      <>
                        ...
                        {disputeQty > 0 && (
                          <div className="mt-3">
                            <label className="text-xs font-semibold text-gray-600 block mb-2">
                              Defective Image {disputeImage && '(Uploaded)'}
                            </label>
                            {isUploading && (
                              <div className="flex items-center gap-2 mb-2 text-blue-600">
                                <Loader size={16} className="animate-spin" />
                            ...
                            {disputeImage ? (
                              <div className="relative group inline-block">
                                ...
                                <button
                                  onClick={() => handleImageUpload(key)}
                                  className="absolute bottom-2 right-2 p-1.5 bg-green-600 text-white rounded-lg opacity-0 group-hover:opacity-100 transition-opacity"
                                >
                                  <Camera size={14} />
                                </button>
                              </div>
                              ...
                              <button
                                onClick={() => handleImageUpload(key)}
                                className="w-32 h-32 rounded-lg border-2 border-dashed border-gray-300 flex flex-col items-center justify-center hover:border-green-400 transition-colors"
                              >
                      ...
                      <>
                        ...
                        {/* Display existing dispute image */}
                        {disputeQty > 0 && disputeImage && (
                          <div className="mt-2 pt-2 border-t border-gray-100">
                            <p className="text-xs font-semibold text-gray-600 mb-2 flex items-center gap-1">
                              <Camera size={12} />
                              Defective Product Image
