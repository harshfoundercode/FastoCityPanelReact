...
//   return (
//     <StockProvider> {/* ✅ Wrap with Provider */}
...
//                   className={`flex flex-col items-center gap-1 px-4 py-1.5 rounded-xl transition-all ${isActive ? 'bg-green-50' : ''}`}
//                 >
//                   <Icon size={22} className={isActive ? 'text-green-800' : 'text-gray-400'} />
//                   <span className={`text-xs font-semibold ${isActive ? 'text-green-800' : 'text-gray-400'}`}>
//                     {tab.label}
//                   </span>
//                 </motion.button>
//               );
  ...
  return (
    <StockProvider>
      ...
      <div className="h-full flex flex-col bg-[#F4F6F9]">
        ...
        <div className="flex-1 overflow-hidden">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              exit={{ opacity: 0, x: -20 }}
              transition={{ duration: 0.2 }}
              className="h-full overflow-auto" // ✅ overflow-auto for scrollable content
            >
              {screens[activeTab]}
            </motion.div>
          </AnimatePresence>
        ...
        <div className="flex-shrink-0 bg-white border-t border-gray-200 shadow-[0_-4px_12px_rgba(0,0,0,0.06)]">
