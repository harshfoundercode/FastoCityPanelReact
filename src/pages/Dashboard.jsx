...
export const Dashboard = () => {
  ...
  // Stats cards configuration with safe fallbacks and navigation
  const statsCards = [
    {
      title: 'Total Orders',
      value: summary.total_orders || 0,
      ...
      bgColor: '#EFF6FF',
      trend: `${completionRate}% Delivered`,
      navigateTo: '/orders', // Navigate to orders page
    },
    {
      title: 'Active Hubs',
      value: summary.total_hubs || 0,
      ...
      bgColor: '#ECFDF5',
      trend: 'Currently Operating',
      navigateTo: '/hubs/all-hubs', // Navigate to all hubs
    },
    {
      title: 'Total Revenue',
      value: `₹${summary.revenue || 0}`,
      ...
      bgColor: '#FFFBEB',
      trend: '↗ Growing',
      navigateTo: '/hubs/hub-performance', // Navigate to hub performance
    },
    {
      title: 'Delivery Boys',
      value: summary.delivery_boys || 0,
      ...
      bgColor: '#F5F3FF',
      trend: 'On Duty',
      navigateTo: '/hubs/all-hubs', // Navigate to hubs to see delivery boys
    },
  ];
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {statsCards.map((stat, index) => (
          <motion.div
            ...
            className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 cursor-pointer transition-all hover:shadow-md"
          >
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <p className="text-sm font-medium" style={{ color: Colors.textGrey1 }}>
            ...
            <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
              ...
              <ChevronRight size={16} style={{ color: Colors.textGrey1 }} />
            </div>
          </motion.div>
        ))}
      </div>

      {/* Transfers Section - Clickable */}
     <motion.div
  initial={{ opacity: 0, y: 20 }}
  animate={{ opacity: 1, y: 0 }}
  transition={{ delay: 0.35 }}
...
>
  <div className="flex items-center justify-between mb-4">
    ...
    <button
      onClick={() => navigate('/incoming-stock')}  // Fixed: changed from '/incoming' to '/incoming-stock'
  ...
  <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
    <div 
      onClick={() => navigate('/incoming-stock')}  // Fixed: changed from '/incoming' to '/incoming-stock'
      className="text-center p-4 rounded-xl bg-purple-50 cursor-pointer hover:opacity-80 transition-opacity"
    >
      <p className="text-2xl font-bold text-purple-600">{summary.total_transfers || 0}</p>
      <p className="text-xs mt-1 text-gray-600">Total Transfers</p>
    </div>
    <div 
      onClick={() => navigate('/incoming-stock')}  // Fixed: changed from '/incoming' to '/incoming-stock'
      className="text-center p-4 rounded-xl bg-blue-50 cursor-pointer hover:opacity-80 transition-opacity"
    >
      <p className="text-2xl font-bold text-blue-600">{summary.approved_transfers || 0}</p>
      <p className="text-xs mt-1 text-gray-600">Approved</p>
    </div>
    <div 
      onClick={() => navigate('/incoming-stock')}  // Fixed: changed from '/incoming' to '/incoming-stock'
      className="text-center p-4 rounded-xl bg-green-50 cursor-pointer hover:opacity-80 transition-opacity"
    >
      <p className="text-2xl font-bold text-green-600">{summary.completed_transfers || 0}</p>
      <p className="text-xs mt-1 text-gray-600">Completed</p>
    </div>
    <div 
      onClick={() => navigate('/incoming-stock')}  // Fixed: changed from '/incoming' to '/incoming-stock'
      className="text-center p-4 rounded-xl bg-yellow-50 cursor-pointer hover:opacity-80 transition-opacity"
    >
      <p className="text-2xl font-bold text-yellow-600">{summary.pending_transfers || 0}</p>
      <p className="text-xs mt-1 text-gray-600">Pending</p>
    </div>
    <div 
      onClick={() => navigate('/incoming-stock')}  // Fixed: changed from '/incoming' to '/incoming-stock'
      className="text-center p-4 rounded-xl bg-red-50 cursor-pointer hover:opacity-80 transition-opacity"
    >
      <p className="text-2xl font-bold text-red-600">{summary.cancelled_transfers || 0}</p>
      <p className="text-xs mt-1 text-gray-600">Cancelled</p>
    </div>
      ...
      {/* Order Progress Section - Clickable */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.3 }}
      ...
      >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <div 
            onClick={() => navigate('/orders')}
            className="text-center p-4 rounded-xl bg-orange-50 cursor-pointer hover:opacity-80 transition-opacity"
          >
            <p className="text-2xl font-bold text-orange-600">{summary.active_orders || 0}</p>
            <p className="text-xs mt-1 text-gray-600">In Progress</p>
          </div>
      ...
      {/* Hubs Section - Individual hubs clickable */}
      {dashboardData.hubs && dashboardData.hubs.length > 0 && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {dashboardData.hubs.map((hub) => (
              <motion.div
                ...
                whileTap={{ scale: 0.98 }}
                onClick={() => navigate(`/hubs/details/${hub.hub_id}`)}
                className="p-4 rounded-xl border border-gray-200 bg-white cursor-pointer hover:shadow-md transition-all"
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <h3 className="font-semibold text-sm capitalize" style={{ color: Colors.textBlack }}>
          ...
          <div className="space-y-3">
            {dashboardData.recent_disputes?.map((dispute, index) => (
              <motion.div
                ...
                animate={{ opacity: 1, x: 0 }}
                transition={{ delay: 0.1 * index }}
                onClick={() => navigate(`/orders/${dispute.order_id}/profile`)}
