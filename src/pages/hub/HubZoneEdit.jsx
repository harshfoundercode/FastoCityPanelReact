                    ...
                    <div className="absolute top-28 left-4 right-4 z-10">
                        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                            {hubZones.map((zone, i) => {
                                ...
                                return (
                                    <button
                                        key={zone.id || `zone-chip-${i}`}
                                        onClick={() => onZoneTap(zone)}
                                        className="flex items-center gap-2 px-3.5 py-2 rounded-full text-xs font-semibold whitespace-nowrap shrink-0 transition-all hover:scale-105"
                                        style={{
                                            backgroundColor: isSelected ? color : 'white',
                                            color: isSelected ? 'white' : '#1E293B',
                                            border: `2px solid ${color}`,
...
// ── Hub Zone Edit Screen (Portal) ─────────────────────────────────────────────
const HubZoneEditScreen = ({ zone, cityZone, onClose, onSave, allHubZones }) => {
    const editMapRef = useRef(null);
    const [editMap, setEditMap] = useState(null);
    const [editMapReady, setEditMapReady] = useState(false);
    const markerRef = useRef(null);
    ...
    const [overlappingZones, setOverlappingZones] = useState([]);
    const [showOutsideBanner, setShowOutsideBanner] = useState(false);
    ...
    // Check if hub zone circle is completely inside city zone
    const isHubCircleInsideCity = (center, hubRadiusKm) => {
        if (!cityZone?.lat) return true;
        const distanceFromCenter = getDistanceFromLatLonInKm(center.lat, center.lng, cityCenter.lat, cityCenter.lng);
        return (distanceFromCenter + hubRadiusKm) <= cityRadiusKm;
    };

    // Check overlap with other hub zones
    const checkHubOverlap = (center, hubRadiusKm, currentZoneId) => {
        if (!allHubZones || allHubZones.length === 0) return { overlaps: false, zones: [] };
        
        const overlapping = allHubZones.filter(otherZone => {
            if (otherZone.id === currentZoneId) return false;
            
            const otherLat = otherZone.latitude || parseFloat(otherZone.lat);
            const otherLng = otherZone.longitude || parseFloat(otherZone.long);
            const otherRadius = otherZone.radiusInKm || parseFloat(otherZone.radiuskm) || 0;
            
            if (!otherLat || !otherLng || isNaN(otherLat) || isNaN(otherLng)) return false;
            
            const distanceBetweenCenters = getDistanceFromLatLonInKm(
                center.lat, center.lng, otherLat, otherLng
            );
            
            return distanceBetweenCenters < (hubRadiusKm + otherRadius);
        });
    ...
    // Run all validations
    const runValidations = (lat, lng, radiusKm) => {
        const center = { lat: parseFloat(lat), lng: parseFloat(lng) };
        const radius = parseFloat(radiusKm);
        
        const outsideCity = !isHubCircleInsideCity(center, radius);
        setIsOutsideCity(outsideCity);
        setShowOutsideBanner(outsideCity);
        
        const overlapResult = checkHubOverlap(center, radius, zone.id);
        setIsOverlapping(overlapResult.overlaps);
        setOverlappingZones(overlapResult.zones);
        setShowOverlapBanner(overlapResult.overlaps && !outsideCity); // Don't show overlap if already outside city
        
        return {
            isValid: !outsideCity && !overlapResult.overlaps,
            outsideCity,
            overlapping: overlapResult.overlaps
        };
    };
    ...
    useEffect(() => {
        ...
        if (cityZone?.lat) {
            cityCircleRef.current = new window.google.maps.Circle({ 
                map: gMap, 
                center: cityCenter, 
        ...
        // Draw all other hub zones on edit map
        if (allHubZones && allHubZones.length > 0) {
            allHubZones.forEach(otherZone => {
                ...
                if (otherLat && otherLng && !isNaN(otherLat) && !isNaN(otherLng)) {
                    new window.google.maps.Circle({
                        map: gMap,
                        center: { lat: otherLat, lng: otherLng },
                        radius: otherRadius * 1000,
                        fillColor: '#EF4444',
                        fillOpacity: 0.05,
                        strokeColor: '#EF4444',
                        strokeOpacity: 0.3,
        ...
        hubCircleRef.current = new window.google.maps.Circle({ 
            map: gMap, 
            center: pos, 
            radius: parseFloat(formData.radiusKm) * 1000, 
            fillColor: '#14532D', 
            fillOpacity: 0.12, 
            strokeColor: '#14532D', 
            strokeOpacity: 0.7, 
        ...
        markerRef.current.addListener('drag', (e) => {
            ...
            const validations = runValidations(lat, lng, radius);
            
            let color;
            if (validations.outsideCity) {
                color = '#EF4444';
            } else if (validations.overlapping) {
                color = '#F59E0B';
            } else {
                color = '#14532D';
            }
            ...
            if (cityCircleRef.current) {
                cityCircleRef.current.setOptions({ 
                    strokeColor: validations.outsideCity ? '#EF4444' : '#3B82F6', 
                    strokeWeight: validations.outsideCity ? 3 : 2.5 
                });
            }
        });
        ...
        runValidations(formData.lat, formData.lng, formData.radiusKm);
    }, [editMapReady]);

    useEffect(() => { 
        if (hubCircleRef.current) {
            const radius = parseFloat(formData.radiusKm);
            hubCircleRef.current.setRadius(radius * 1000);
            runValidations(formData.lat, formData.lng, radius);
        }
    }, [formData.radiusKm]);
    ...
    const onLocationPicked = (lat, lng) => {
        ...
        if (validations.outsideCity) {
            toast.error('Hub zone circle is going outside the city zone!');
        } else if (validations.overlapping) {
            toast.error(`Overlapping with ${overlappingZones.length} other hub zone(s)!`);
        }
    };
    ...
    const resetToCenter = () => {
        ...
        editMap?.panTo({ lat, lng });
        runValidations(lat, lng, formData.radiusKm);
        toast.success('Location reset to city zone center.');
    };
    ...
    const handleSearch = async (q) => {
        ...
        setSearchLoading(true);
        try { 
            const r = await apiClient.get(`place_autocomplete?query=${encodeURIComponent(q)}`); 
    ...
    const handleSubmit = async () => {
        ...
        if (!isHubCircleInsideCity(center, radius) && cityZone?.lat) {
            toast.error('Cannot save: Hub zone circle extends outside the city zone boundary');
            return;
        }
        
        const overlapResult = checkHubOverlap(center, radius, zone.id);
        if (overlapResult.overlaps) {
            const zoneNames = overlapResult.zones.map(z => z.name).join(', ');
            toast.error(`Cannot save: Zone overlaps with: ${zoneNames}`);
            return;
        }
    ...
    const isSaveDisabled = isSubmitting || isOutsideCity || isOverlapping;
    ...
    return ReactDOM.createPortal(
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
                <div className="lg:w-[55%] h-64 lg:h-full relative shrink-0 border-b lg:border-b-0 lg:border-r border-gray-200">
                    <div ref={editMapRef} className="w-full h-full absolute inset-0" />
                    {showOutsideBanner && <div className="absolute top-0 left-0 right-0 bg-linear-to-r from-red-500 to-red-600 text-white px-4 py-2.5 text-xs font-bold flex items-center gap-2 z-10 shadow-lg"><AlertTriangle size={16} /> Hub zone circle extends OUTSIDE the city boundary!</div>}
                    {showOverlapBanner && <div className="absolute top-0 left-0 right-0 bg-linear-to-r from-orange-500 to-amber-500 text-white px-4 py-2.5 text-xs font-bold flex items-center gap-2 z-10 shadow-lg"><AlertTriangle size={16} /> Overlapping with {overlappingZones.length} other zone(s): {overlappingZones.map(z => z.name).join(', ')}</div>}
                    <div className={`absolute ${(showOutsideBanner || showOverlapBanner) ? 'top-12' : 'top-3'} left-3 z-10`}>
                        <span className={`px-3 py-1.5 rounded-full text-xs font-bold shadow-lg bg-white/95 backdrop-blur-sm border ${isOutsideCity ? 'text-red-600 border-red-200' : isOverlapping ? 'text-orange-600 border-orange-200' : 'text-green-700 border-green-200'}`}>
                            {isOutsideCity ? '🔴 Outside city zone' : isOverlapping ? '🟠 Overlapping zones' : '✅ Valid position'}
                        </span>
                    </div>
                    {cityZone?.lat && <div className="absolute bottom-3 left-3 z-10"><span className="px-2.5 py-1.5 rounded-lg text-xs font-semibold bg-white/95 shadow border border-blue-200 text-blue-700">● City boundary • {cityRadiusKm} km</span></div>}
                </div>
                <div className="flex-1 overflow-y-auto p-5 lg:p-6 space-y-5">
                    <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
                        ...
                        <div className="relative">
                            ...
                            {suggestions.length > 0 && (
                                <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl border shadow-2xl z-30 max-h-64 overflow-y-auto">
                                    {suggestions.map((s, i) => (
                                        <button key={s.place_id || `suggestion-${i}`} onClick={() => selectPlace(s.place_id)} className="w-full px-4 py-3.5 text-left hover:bg-gray-50 flex items-center gap-3 border-b border-gray-50 last:border-0">
                                            <div className="w-9 h-9 rounded-xl bg-green-50 flex items-center justify-center shrink-0"><MapPin size={16} className="text-green-700" /></div>
                                            <div className="min-w-0"><p className="text-sm font-bold text-gray-800 truncate">{s.main_text || s.description}</p>{s.secondary_text && <p className="text-xs text-gray-400 truncate mt-0.5">{s.secondary_text}</p>}</div>
                                        </button>
                                    ))}
                    ...
                    {showOutsideBanner && cityZone?.lat && (
                        <div className="p-4 bg-linear-to-r from-red-50 to-orange-50 border-2 border-red-200 rounded-2xl flex items-start gap-3">
                            <div className="w-9 h-9 rounded-xl bg-red-100 flex items-center justify-center shrink-0"><AlertTriangle size={18} className="text-red-600" /></div>
                            <div className="flex-1"><p className="text-sm font-extrabold text-red-700">Outside City Zone</p><p className="text-xs text-red-600 mt-1">Hub zone circle extends beyond the city boundary. Reduce radius or move center inside.</p></div>
                            <button onClick={resetToCenter} className="px-3.5 py-2 bg-red-600 text-white rounded-xl text-xs font-bold hover:bg-red-700 transition-colors shrink-0">Reset</button>
                        </div>
                    )}
                    {showOverlapBanner && (
                        <div className="p-4 bg-linear-to-r from-orange-50 to-amber-50 border-2 border-orange-200 rounded-2xl flex items-start gap-3">
                            <div className="w-9 h-9 rounded-xl bg-orange-100 flex items-center justify-center shrink-0"><AlertTriangle size={18} className="text-orange-600" /></div>
                            <div className="flex-1"><p className="text-sm font-extrabold text-orange-700">Zone Overlap Detected</p><p className="text-xs text-orange-600 mt-1">Overlapping with: {overlappingZones.map(z => z.name).join(', ')}. Adjust position or radius.</p></div>
                            <button onClick={resetToCenter} className="px-3.5 py-2 bg-orange-600 text-white rounded-xl text-xs font-bold hover:bg-orange-700 transition-colors shrink-0">Reset</button>
                        </div>
                    )}
                    <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
                        <label className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3 block">Coordinates & Radius</label>
                    ...
                    <button onClick={() => {
                        ...
                        className={`w-full py-3.5 rounded-2xl text-white text-sm font-extrabold flex items-center justify-center gap-2.5 shadow-lg transition-all active:scale-[0.98] ${isOutsideCity ? 'bg-red-500' : isOverlapping ? 'bg-orange-500' : 'bg-linear-to-r from-green-700 to-green-800 hover:from-green-800 hover:to-green-900'} disabled:opacity-60`}>
                        {isSubmitting ? <Loader size={20} className="animate-spin" /> : <Save size={20} />}
                        {isOutsideCity ? '⚠ Cannot Save — Outside City Zone' : isOverlapping ? '⚠ Cannot Save — Zone Overlap' : 'Save Changes'}
                    </button>
                </div>
            </div>
        </motion.div>,
