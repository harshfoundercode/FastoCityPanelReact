...
// import { EditHubDrawer } from '../hub/EditHubDetails'; 
...
export const AllHubs = () => {
  ...
  // State for copy button - track which hub is being copied
  const [copyingHubId, setCopyingHubId] = useState(null);
  ...
  const handleCopyCredentials = async (hub, e) => {
    e.stopPropagation();
    
    const hubId = hub.hub_id;
    setCopyingHubId(hubId);
    
    try {
      const profile = await fetchHubProfile(hubId);
